...
showPotential (a1:as) = "(" ++ (show a1) ++ "," ++ (showPotential as)

showPotentialDouble::[Double] -> String
showPotentialDouble (a1:[]) = (show a1) ++ ")"
showPotentialDouble (a1:as) = "(" ++ (show a1) ++ "," ++ (showPotentialDouble as)
...
instance ShowA Double where
  ...
  showAl p = "@" ++ showPotentialDouble p
-- s
...
prettyType _ (TyList q p t) = text ("L" ++ showA q ) <> parens (text (showAl p)) <> parens (prettyType 0 t)
